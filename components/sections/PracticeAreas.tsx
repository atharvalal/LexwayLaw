...
import { Building2, Gavel, Home, Users, Scale, ArrowRight, Shield } from 'lucide-react'
  ...
  return (
    <section className="section-padding bg-[#060110]">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 gap-px bg-[#FFFFFF]/10 md:grid-cols-2 lg:grid-cols-3">
          {areas.map((area, i) => (
            <motion.div
              key={area.title}
            ...
            >
              <Link
                href="/practice-areas"
                className="group relative block h-full overflow-hidden bg-[#0B011C] p-6 transition-all duration-400 hover:bg-[#0d0330] sm:p-7 md:p-8"
              >
                {/* Hover accent */}
                ...
                <h3
                  className="mb-3 text-xl font-serif text-white transition-colors duration-300 group-hover:text-[#FFFFFF]"
                  style={{ fontFamily: 'Playfair Display, Georgia, serif' }}
                >
                  {area.title}
                </h3>
                <p className="mb-6 text-sm leading-relaxed text-white/40">{area.description}</p>
